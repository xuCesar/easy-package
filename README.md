...
Easy Package 是面向会用终端、偏好用 GUI 收拢开发环境的 macOS 开发者的本机环境面板：统一查看全局软件包、PATH 与运行时状态，并以「先预检、后确认」的安全方式执行受控升级。项目分析是按需使用的第二个工作区。
...
- 概览页聚合本机「需关注问题」，包括环境健康项与可更新软件包；项目级问题在项目分析中查看
...
2. **添加项目（可选）**：需要项目分析时，再在「项目」页添加代码目录（如 `~/Code`)，依赖与运行时分析会随扫描更新；可配置遍历深度与忽略目录。
