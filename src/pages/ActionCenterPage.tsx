...
export function ActionCenterPage(props: ActionCenterPageProps) {
  ...
  return (
    <>
      <PageHeader title="操作中心" description="通过后端固定白名单执行 Homebrew Formula、npm 与 pnpm 全局包操作；每次修改都先预检、确认并在完成后重新扫描。" />
      ...
      <div className="view-tabs action-tabs" role="tablist" aria-label="软件包操作">
        {(Object.keys(actionLabels) as PackageAction[]).map((item) => <button key={item} role="tab" aria-selected={action === item} className={action === item ? "view-tab view-tab--active" : "view-tab"} onClick={() => changeAction(item)} disabled={props.isExecuting}>{actionDisplay(managerId, item)}</button>)}
      </div>
      {capability ? <section className={`panel action-capability action-capability--${capability.ready ? "ready" : "blocked"}`} aria-label="动作可用性"><div className="panel__header"><h2>{capability.ready ? "可以生成计划" : "当前操作被阻止"}</h2><span className={`action-status action-status--${capability.ready ? "succeeded" : "failed"}`}>{capability.ready ? "就绪" : "阻止"}</span></div><div className="capability-checks">{capability.checks.map((check) => <div key={`${check.code}:${check.title}`} className={`capability-check capability-check--${check.status}`}><strong>{check.title}</strong><span>{check.detail}</span><code>{check.code}</code></div>)}</div></section> : null}
      ...
      <section className="panel action-builder" aria-label="操作预检设置">
        ...
        <div className="action-builder__body">
          ...
          <button className="button button--primary" onClick={() => void createPlan()} disabled={capability?.ready !== true || recoveryRequired || props.isPlanning || props.isExecuting || (action !== "cleanup" && targets.length === 0)}>{props.isPlanning ? "正在预检…" : "生成操作计划"}</button>
        </div>
      </section>
      ...
      {props.plan ? <section className="panel action-plan" aria-label="待确认操作计划">
        ...
        <div className="action-plan__body">
          <div className="action-plan__summary"><div><span>管理器</span><strong>{managerLabels[props.plan.managerId]}</strong></div><div><span>操作</span><strong>{actionDisplay(props.plan.managerId, props.plan.action)}</strong></div><div><span>目标</span><strong>{props.plan.targets.join("、") || `${managerLabels[props.plan.managerId]} 缓存`}</strong></div><div><span>网络</span><strong>{props.plan.requiresNetwork ? "需要访问软件源" : "不主动联网"}</strong></div></div>
          ...
          <div><h3>安全检查</h3><ul className="plan-lines">{props.plan.checks.map((check) => <li key={`${check.code}:${check.title}`}>{check.title}：{check.detail}</li>)}</ul></div>
      ...
      <section className="panel action-audit" aria-label="操作审计记录">
        <div className="panel__header"><h2>最近操作审计</h2><div className="audit-filter"><select aria-label="审计管理器筛选" value={auditManager} onChange={(event) => setAuditManager(event.target.value as typeof auditManager)}><option value="all">全部管理器</option><option value="homebrew">Homebrew</option><option value="npm">npm</option><option value="pnpm">pnpm</option></select><span className="count-label">{filteredAudit.length}</span></div></div>
        {filteredAudit.length ? <div className="audit-list">{filteredAudit.map((record) => <article key={record.actionId}><span className={`action-status action-status--${record.status}`}>{statusLabels[record.status]}</span><div><strong>{managerLabels[record.managerId]} · {actionDisplay(record.managerId, record.action)} · {record.targets.join("、") || "缓存"}</strong><code>{record.commandPreview}</code>{record.observedOutcome ? <small>{outcomeLabels[record.observedOutcome]}</small> : null}{record.status === "running" || record.rescanRequired ? <small>需要重新扫描后确认实际状态</small> : null}{expandedAuditId === record.actionId ? <div className="audit-detail"><p>{record.error ?? "命令没有报告错误。"}</p>{record.evidence.map((item) => <span key={item}>{item}</span>)}{record.logs.map((line, index) => <code key={`${index}:${line}`}>{line}</code>)}</div> : null}</div><div className="audit-actions"><time>{new Date(record.finishedAt).toLocaleString()}</time><button className="button button--secondary" onClick={() => setExpandedAuditId((current) => current === record.actionId ? undefined : record.actionId)}>{expandedAuditId === record.actionId ? "收起详情" : "查看详情"}</button>{record.status === "running" || record.rescanRequired ? <button className="button button--primary" onClick={() => void props.onReconcile(record.actionId)} disabled={props.isReconciling}>重新核对</button> : null}</div></article>)}</div> : <EmptyState icon="history" title="没有匹配的写操作记录" description="完成受控操作后，结果会保存在本机审计记录中。" />}
      </section>
    </>
  );
}
