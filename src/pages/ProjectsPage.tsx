...
export function ProjectsPage({ projects, workspaces, scanRoots, onAddRoot, onRemoveRoot, onRefresh }: ProjectsPageProps) {
  ...
  return (
    <>
      ...
      {projects.length ? <div className="project-groups">{groups.workspaces.map(({ workspace, projects: members }) => <section key={workspace.path}><div className="project-group__header"><div><h2>{workspace.name}</h2><p>{workspace.ecosystem} 工作区 · {members.length} 个成员项目</p></div></div><div className="project-grid">{members.map((project) => <ProjectCard key={project.path} project={project} />)}</div></section>)}{groups.standalone.length ? <section><div className="project-group__header"><div><h2>独立项目</h2><p>未归属已识别工作区的项目</p></div></div><div className="project-grid">{groups.standalone.map((project) => <ProjectCard key={project.path} project={project} />)}</div></section> : null}</div> : <section className="panel project-empty"><button onClick={() => void chooseRoot()} disabled={isChoosing}><Icon name="plus" /><strong>{isChoosing ? "选择中…" : "添加扫描目录"}</strong><span>选择一个开发目录后，Easy Package 会识别项目元数据。</span></button></section>}
    </>
  );
}
