...
export function ProjectsPage({ projects, workspaces, scanRoots, scanSettings, onAddRoot, onRemoveRoot, onUpdateSettings, onExportReport, onRefresh }: ProjectsPageProps) {
  ...
  const saveSettings = async (nextSettings: ScanSettings) => {
    ...
    try {
      ...
      setActionNotice("扫描范围已更新，项目、依赖与运行时关联已重新计算。");
    } catch (error) {
      setActionError(error instanceof Error ? error.message : String(error));
    } finally {
    ...
    <>
      ...
      <section className="scan-settings panel" aria-label="扫描范围设置">
        ...
        <div className="scan-settings__body">
          ...
          <div className="scan-setting"><div><strong>联网策略</strong><p>离线模式不会执行可能访问 registry 的更新检查；修改后需重新扫描生效。</p></div><div className="scan-setting__control"><label className="sr-only" htmlFor="network-policy">联网策略</label><select id="network-policy" value={scanSettings.networkPolicy} onChange={(event) => void saveSettings({ ...scanSettings, networkPolicy: event.target.value as ScanSettings["networkPolicy"] })} disabled={isSavingSettings}><option value="offline">离线</option><option value="registry">允许 registry 检查</option></select></div></div>
