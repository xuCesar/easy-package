...
export function EnvironmentPage({ data }: { data: EnvironmentScan }) {
  ...
  return (
    <>
      <PageHeader title="环境" description="检查包管理器路径、版本、缓存和 PATH 优先级。诊断信息不会自动修复。" />
      <div className="environment-layout">
        ...
        <section className="panel">
          <div className="panel__header"><h2>PATH 解析</h2><span className="quiet-label">当前优先项</span></div>
          <div className="path-list">{data.pathObservations.map((item) => (
            <article className="path-item" key={item.command}><div><code>{item.command}</code>{item.hasConflict ? <span className="conflict-label">多个来源</span> : null}</div><strong>{item.activePath ?? "未找到"}</strong>{item.alternatives.length ? <p>其他路径：{item.alternatives.join(" · ")}</p> : <p>未发现其他来源</p>}</article>
          ))}</div>
        </section>
