...
import type { ReportExportResult, ReportFormat, SnapshotChangeEntity, SnapshotChangeKind, SnapshotComparison, SnapshotSummary } from "../types";
    ...
    <>
      ...
      {summaries.length ? <>
        <section className="metrics history-metrics" aria-label="历史摘要">
          <div className="metric"><span className="metric__icon"><Icon name="history" /></span><div><strong>{summaries.length}</strong><span>保留快照</span></div></div>
          <div className="metric"><span className="metric__icon"><Icon name="check" /></span><div><strong>{comparison?.addedCount ?? 0}</strong><span>新增项</span></div></div>
          <div className="metric"><span className="metric__icon"><Icon name="warning" /></span><div><strong>{comparison?.changedCount ?? 0}</strong><span>变化项</span></div></div>
          <div className="metric"><span className="metric__icon"><Icon name="trash" /></span><div><strong>{comparison?.removedCount ?? 0}</strong><span>移除项</span></div></div>
        </section>
        ...
        {summaries.length === 1 ? <section className="panel"><p className="quiet-message">至少需要两次成功扫描，才能显示环境变化。</p></section> : null}
        {comparison ? <>
          <div className="toolbar" role="search">
            <label className="select-field">类型<select aria-label="变化类型" value={entity} onChange={(event) => setEntity(event.target.value as SnapshotChangeEntity | "all")}><option value="all">全部类型</option>{Object.entries(entityLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
            <label className="select-field">结果<select aria-label="变化结果" value={kind} onChange={(event) => setKind(event.target.value as SnapshotChangeKind | "all")}><option value="all">全部结果</option>{Object.entries(kindLabel).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
            <span className="toolbar__count">{changes.length} 项变化</span>
            <label className="select-field">报告<select aria-label="变化报告格式" value={reportFormat} onChange={(event) => setReportFormat(event.target.value as ReportFormat)}><option value="markdown">Markdown</option><option value="json">JSON</option></select></label>
            <button className="button button--secondary" onClick={() => void exportComparison()} disabled={isExporting}>{isExporting ? "导出中…" : "导出变化报告"}</button>
          </div>
          <section className="panel history-panel">
            {changes.length ? <div className="history-list">{changes.map((change) => <article className="history-item" key={`${change.entity}:${change.kind}:${change.key}`}><div><span className={`history-badge history-badge--${change.kind}`}>{kindLabel[change.kind]}</span><span className="manager-chip">{entityLabel[change.entity]}</span><strong>{change.title}</strong><p>{change.description}</p></div></article>)}</div> : <EmptyState icon="history" title="没有匹配的变化" description="调整筛选条件，或选择其他快照进行比较。" />}
          </section>
        </> : null}
      </> : <section className="panel"><EmptyState icon="history" title={isLoading ? "正在读取快照历史" : "尚无环境快照"} description="完成一次成功扫描后，Easy Package 会在本机保留快照以供后续比较。" /></section>}
    </>
  );
}
