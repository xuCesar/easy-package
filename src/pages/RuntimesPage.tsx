...
export function RuntimesPage({
  ...
  assessments: RuntimeRequirementAssessment[];
}) {
  ...
  const filteredAssessments = assessments.filter(
    (item) =>
      ...
      (!onlyIssues || item.status === "missing" || item.status === "mismatch"),
  );
  ...
  return (
    <>
      <PageHeader
        title="运行时"
        description="只读发现 Node.js、Python 与 Rust 安装来源，并关联项目声明；不会安装、切换或删除运行时。"
      />
      ...
      <div className="runtime-layout">
        ...
        <section className="panel">
          <div className="panel__header">
            <h2>项目关联</h2>
            <span className="count-label">{filteredAssessments.length}</span>
          </div>
          {filteredAssessments.length ? (
            <div className="runtime-assessments">
              {filteredAssessments.map((item) => (
                <article
                  className={`runtime-assessment runtime-assessment--${item.status}`}
                  key={`${item.projectPath}:${item.runtime}`}
                >
                  <div>
                    <strong>{item.projectName}</strong>
                    <code title={item.projectPath}>{item.projectPath}</code>
                  </div>
                  <div>
                    <span>
                      {item.runtime} {item.requirement}
                    </span>
                    <b>当前 {item.activeVersion ?? "未发现"}</b>
            ...
            <EmptyState
              icon="check"
              title="没有关联异常"
              description="当前筛选范围内的项目运行时声明均有本机安装可供参考。"
            />
          )}
        </section>
      </div>
