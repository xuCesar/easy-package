...
export function LogsPage({ logs }: { logs: TaskLog[] }) {
  ...
  const filteredLogs = useMemo(
    () =>
      [...logs]
        .filter(
          (log) => (category === "all" || log.category === category) && (status === "all" || log.status === status),
        )
        .sort((left, right) => new Date(right.timestamp).getTime() - new Date(left.timestamp).getTime()),
    [category, logs, status],
  );
    ...
    <>
      ...
      <section className="panel logs-panel">
        <div className="panel__header">
          <h2>扫描记录</h2>
          <span className="quiet-label">按时间倒序</span>
        </div>
        {filteredLogs.length ? (
          <div className="log-list">
            {filteredLogs.map((log) => (
              <article className={`log-row log-row--${log.status}`} key={log.id}>
                <time dateTime={log.timestamp}>{formatLogTime(log.timestamp)}</time>
                <div className="log-row__main">
                  <div>
                    <span className="log-category">{categoryLabel[log.category]}</span>
                    <span className="log-status">{statusLabel[log.status]}</span>
                    {log.managerId ? <span className="manager-chip">{managerLabel[log.managerId]}</span> : null}
                  </div>
                  <strong>{log.message}</strong>
                  {log.exitCode !== undefined ? <small>退出码 {log.exitCode}</small> : null}
                </div>
                {log.output ? (
                  <button className="text-button" onClick={() => void copyOutput(log)}>
                    {copied === log.id ? "已复制" : "复制输出"}
                  </button>
                ) : null}
              </article>
            ))}
          ...
          <EmptyState icon="logs" title="没有匹配的日志" description="调整日志类别或状态筛选条件后重试。" />
        )}
      </section>
    </>
  );
