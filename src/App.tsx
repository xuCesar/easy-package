...
const diagnosticLabels: Record<(typeof diagnosticPages)[number], string> = { environment: "环境", analysis: "项目分析", runtimes: "运行时", history: "历史", logs: "日志" };
  ...
  if (!data && isLoading) {
    ...
    content = (
      <>
        ...
        {page === "overview" ? <OverviewPage data={data} isLoading={isLoading} scanProgress={scanProgress} comparison={history.comparison} onRefresh={() => void refresh()} onCancel={() => void cancelScan()} onNavigate={navigate} onOpenAnalysis={openAnalysis} /> : null}
        ...
        {page === "analysis" ? <ProjectAnalysisPage view={analysisView} onChangeView={setAnalysisView} insights={data.dependencyInsights} projects={data.projects} workspaces={data.workspaces} scanRoots={data.scanRoots} ignoredDirectoryNames={data.scanSettings.defaultIgnoredDirectoryNames} onLoadGraph={getProjectDependencyGraph} onLoadReport={getProjectSupplyChainReport} onExportSbom={exportProjectSbom} /> : null}
